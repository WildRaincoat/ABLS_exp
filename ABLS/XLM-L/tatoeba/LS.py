...
from utils.common45 import (
    ...
    # ✅ token text helpers / filters
    is_symbol_or_digit_only,
    is_short_latin_fragment,
)
...
OUTPUT_DIR = "/ACL_exp/Models_LPD/XLM-L/tatoeba45"
...
# ✅ Tatoeba 很大：每种语言最多抽 2000 条 all 样本
...
def load_texts_all_and_small_tatoeba():
    ...
    # 先按 common.py 的 loader 读入并 shuffle（每语种内部）
    ...
    for lab in TARGET_LID_LABELS:
        ...
        # small 确保最多 100（loader 已经做了，这里再保险）
...
# ==========================
# Stage1: MLM pseudo-generation (sequential full sentence)
# ==========================
def pseudo_reply_mlm_full_sentence_sequential(
    ...
    """
    - 从左到右逐位置 mask（跳过 special），取 top-1 预测并回填
    - 输出长度与输入一致，decode 整条序列作为伪句子
    """
...
def compute_escape_sigma_by_sentence_confusion(
    ...
    """
    For each sample:
      find the smallest sigma such that detected reply language != src_lang.
    Return mean_sigma.
    """
...
# ==========================
# Stage2: mask position filter (NEW)
# ==========================
def is_bad_mask_token_text(text: str) -> bool:
    """
    过滤规则（按你的要求“简单”）：
    - 纯符号/标点/数字：is_symbol_or_digit_only(text) -> True
    - strip 后过短：len(strip) <= 1
    - 短拉丁碎片（<=2字母）：is_short_latin_fragment
    """
    t = (text or "")
    ts = t.strip()
    ...
    if len(ts) <= 1:
        return True
    if is_symbol_or_digit_only(ts):
        return True
    if is_short_latin_fragment(ts, max_len=2):
        return True

    return False


def pick_random_mask_position_with_filter(tokenizer, input_ids_1d: torch.Tensor, special_ids: set):
    """
    从非 special 的位置里随机选一个 mask 位置，并确保该 token 不是：
    - 标点/符号/数字
    - 过短 token
    """
    cands = []
    ...
    for i in range(L):
        ...
        if tid in special_ids:
            continue
        text = tokenizer.decode([tid], skip_special_tokens=True) or ""
        if is_bad_mask_token_text(text):
            continue
        cands.append(i)
...
def main():
    ...
    # 4) dataset (Tatoeba)
    lang_to_all_texts, lang_to_small_texts = load_texts_all_and_small_tatoeba()
    ...
    for sigma in sorted(sigma_to_langs.keys()):
        ...
        for src_lang in langs_at_sigma:
            ...
            if not texts:
                print(f"[Stage2 σ={sigma:.4f}] {src_lang} has 0 samples -> zeros")
                ...
                save_similarity_progress(
                    ...
                    notes="Tatoeba; Stage1: mean first-confuse sigma via sequential full-sentence MLM; Stage2: token-prob aggregation NO renormalize; mask position filtered (no punct/symbol/digit/too-short).",
                )
                continue
            ...
            for text in texts:
                ...
                # ✅ Stage2：随机挑一个非 special 位置 mask + filter（新加）
                mask_pos = pick_random_mask_position_with_filter(tokenizer, ids_1d, special_ids)
                if mask_pos is None:
                    continue

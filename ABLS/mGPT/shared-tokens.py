...
OUTPUT_DIR = "/ACL_exp/Models_LPD/mGPT"
os.makedirs(OUTPUT_DIR, exist_ok=True)

SHARED_TOKENS_JSON = os.path.join(
    OUTPUT_DIR,
    "mgpt_shared_tokens_v2_45.json"
)
...
def main():
    ...
    # -------------------------------
    # 逐语言 tokenize & count（只统计 TOTAL，不再统计句首）
    # -------------------------------
    ...
    for lang_code in active_langs:
        ...
        for text in tqdm(texts, desc=f"  Tokenizing {lang_code}"):
            ...
            for tid in ids:
                ...
                if ok is None:
                    surf = token_surface_via_decode(tokenizer, tid)  # 用 decode 的真实字符

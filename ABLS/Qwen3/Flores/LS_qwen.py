...
OUTPUT_DIR = "/ACL_exp/Models_LPD/Qwen3/Flores45"
...
# ==========================
# Model builder (CausalLM, load once + dynamic sigma)
# ==========================
...
def compute_escape_sigma_parallel_roundscan(
    ...
    for lang in src_langs:
        ...
        pending = [i for i in range(len(bs))]
        trackers[lang] = {"bs": bs, "pending": pending}
        confused_counts_by_lang[lang] = {}

    if all(len(trackers[l]["pending"]) == 0 for l in trackers):
        return {l: SIGMA_GRID[-1] for l in src_langs}, confused_counts_by_lang
...
def causal_first_generated_token_probs_full_prompt(
    ...
    直接把完整输入 prompt 喂给 CausalLM，然后取最后位置 logits：
...
def main():
    ...
    # 4) token -> language weights (cache)
    token_lang_weights = build_token_language_weights(
        tokenizer=tokenizer,  # ✅ 直接用同一个 tokenizer
    ...
    #   ✅ 模型只加载一次，sigma 动态切换（controller.set_sigma）
    #   ✅ 多语言并行 round-robin 扫 sigma
    # ==================
    ...
    #   ✅ 变化：Stage2 改为“完整 prompt 的 next-token 分布”
    #   ✅ 不再反复加载模型；按语言的 escape_sigma 动态 set_sigma
    # ==================
